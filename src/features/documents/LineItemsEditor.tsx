...
export default function LineItemsEditor({
  ...
  return (
    <div>
      <div className="hidden grid-cols-[minmax(0,3fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)_2rem] gap-2 px-1 pb-1 text-[11px] font-semibold uppercase tracking-wide text-stone-500 md:grid">
      ...
      <div className="space-y-3 md:space-y-2">
        {lines.map((l, idx) => (
          <div key={l.key} className="grid grid-cols-2 gap-2 rounded-md bg-stone-50 p-3 md:grid-cols-[minmax(0,3fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)_2rem] md:items-center md:bg-transparent md:p-0">
            ...
            <div className="tabular text-right text-sm text-stone-700 md:pr-1">{formatINR(draftAmount(l))}</div>
            <button
              type="button"
              className="justify-self-end rounded-md p-1.5 text-stone-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-40"
      ...
      {units.length === 0 && (
        <p className="mt-2 text-xs text-stone-500">No units are defined yet — an administrator can add the business's units under Settings → Stock. A unit is optional.</p>
      )}
    </div>
  )
}
